...
const tpChecker = home => {
    let sheet = 500 * home.tp
    let p = home.people * 57
    let count = 1
    while ((sheet -= p) > p) {
        count++
    }
    ...
    if (count < 14) {
        return `Your TP will only last ${count} days, buy more!`
    } else {
        return `Your TP will last ${count} days, no need to panic!`
    }

    const days = Math.floor((500 * home.tp) / (57 * home.people))
    days
    return days < 14
